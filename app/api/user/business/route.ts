...
export async function GET(req: Request) {
  try {
    ...
    if (!user || !user.business) {
      ...
      if (!business) {
        return new Response(JSON.stringify({ business: null }), {
          status: 200,
          headers: {
            "Content-Type": "application/json",
            "Cache-Control": "no-cache, no-store, must-revalidate",
    ...
    // When returning the response, add cache control headers
    return new Response(JSON.stringify({ business: user.business }), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
        "Cache-Control": "no-cache, no-store, must-revalidate",
